Proof-of-Concept implementation for an hash-based MVBA.

The code is forked from the implmenetaion of [Dumbo-NG](https://github.com/yylluu/Dumbo_NG).

This codebase also includes PoC implementation for FIN-MVBA in [FIN: Practical Signature-Free Asynchronous Common Subset in Constant Time](https://dl.acm.org/doi/10.1145/3576915.3616633).

## Pre-requisites

It is recommended to prepare an isolated and reusable environment for this codebase, as you may prefer not to pollute the system environemnt by installing random libraries in the system path.

On a local machine, `docker` is recommended.
...
The running `docker` environment has a `sshd` service binding the port `20022` on the `localhost`, from which the environment may be accessed. The default user name is `root` without password.

## Other setup

If you do wish to continue on a non-isolated environment, ideally an Ubuntu OS is preferred.

Please consult the [docker/env.Dockerfile](docker/env.Dockerfile) file to understand how dependencies should be installed.

## How to run tests

Once a working test environment is ready, one can run local experiments via three available `bash` scripts. Please read the comments in these files and make use of them.

```
test_hmvba.sh
test_dumbomvbastar.sh
test_finmvba.sh
```
